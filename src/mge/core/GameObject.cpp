...
GameObject::GameObject(const std::string& pName, const glm::vec3& pPosition)
	...
	_mesh(nullptr), _behaviour(nullptr), _material(nullptr), _world(nullptr), _isStatic(true)
...
void GameObject::setCollider(Collider * pBox)
{
	_boundingBox = pBox;
}

void GameObject::createCollider(bool isOBB)
{
	if (isOBB == false)
	{
		glm::vec3 pos = getWorldPosition();
		glm::vec3 dif{ 1.0f,1.0f,1.0f }; //I know my objects are 2x2x2 for the vertices 
		_boundingBox = new AABB(pos - dif, pos + dif);
		_boundingBox->m_gameObject = this;
	}
	else
	{
		glm::vec3 pos = getWorldPosition();
		glm::vec3 dif{ 1.0f,1.0f,1.0f }; //I know my objects are 2x2x2 for the vertices 
		_boundingBox = new OBB(pos - dif, pos + dif);
		_boundingBox->m_gameObject = this;
		rotate(45, glm::vec3(1, 0, 0));
		glm::vec4 nMax(_boundingBox->Max, 1);
		glm::vec4 nMin(_boundingBox->Min, 1);
		_boundingBox->Max = nMax * getTransform();
		_boundingBox->Min = nMin * getTransform();
	}
}
...
bool GameObject::update(float pStep)
{
	//make sure behaviour is updated after worldtransform is set
	if (_behaviour) {
		return _behaviour->update(pStep);
	}
