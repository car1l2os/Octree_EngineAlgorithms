...
void AbstractGame::_update(float pStep) {
	...
	if (_octree != nullptr)
	{
		...
		list<IntersectionRecord*>* l_list = _octree->GetIntersections();
		if (_octree->static_intersections_done == false)
		{
			list<IntersectionRecord*>* static_l_list = _octree->GetStaticIntersections();
			loop_collisions = l_list->size() + static_l_list->size();

			for (std::list<IntersectionRecord*>::iterator it = static_l_list->begin();it != static_l_list->end();it++)
			{
				delete *it;
				*it = nullptr;
			}
			delete static_l_list;
			static_l_list = nullptr;
		}
		else
		{
			loop_collisions = l_list->size();
		}

		for (std::list<IntersectionRecord*>::iterator it = l_list->begin();it != l_list->end();it++)
		{
			delete *it;
			*it = nullptr;
		}
		...
		TestManager::per_loop_collisions = loop_collisions;
	}
}
